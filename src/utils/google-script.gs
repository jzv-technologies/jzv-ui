...
function handleRequest(e) {
  try {
    ...
    if (action === "invalidate-form-cache") {
  ...
  for (let i = 1; i < data.length; i++) {
    if (String(data[i][idxUUID]).trim() === uuid.trim()) {
      const mapping = {
        ...
        idPattern: idxIdPattern !== -1 ? String(data[i][idxIdPattern]).trim() : "ID-XXXXX"
      };
...
function getFormConfig(uuid) {
  ...
  console.log(`Action [config]: Cache MISS for ${formCacheKey}. Reading from Sheet.`);
  const ss = SpreadsheetApp.openById(CONFIG_SPREADSHEET_ID);
...
function submitFormData(body) {
  ...
  const props = PropertiesService.getScriptProperties();
  let cachedHeaders = props.getProperty(headerKey);
  let headers = cachedHeaders ? JSON.parse(cachedHeaders) : null;

  if (!headers) {
    console.log(`Action [submit]: Header cache MISS for "${targetSheetName}". Reading sheet.`);
    headers = sheet.getDataRange().getValues()[0].map((h) => String(h).trim());
    if (headers.length === 1 && headers[0] === "") {
      headers = ["id"]; 
    }
    props.setProperty(headerKey, JSON.stringify(headers));
  }

  // Check for new fields
  const formKeys = Object.keys(formData);
  const newFields = formKeys.filter(key => key !== "id" && !headers.includes(key));
  
  if (newFields.length > 0) {
    headers = [...headers, ...newFields];
    sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
    props.setProperty(headerKey, JSON.stringify(headers));
    console.log(`Action [submit]: Expanded headers and updated cache for "${targetSheetName}". Added: ${newFields.join(", ")}`);
  }
  ...
  console.log(`Action [submit]: Row appended successfully. ID: ${formattedId}`);
...
function searchFormData(body) {
  ...
  if (!sheet) return jsonResponse(null, `Sheet "${mapping.dataSheetName}" not found`);
  ...
  const filterSpecs = criteriaKeys.map(key => {
  ...
  if (criteriaKeys.length > 0 && filterSpecs.length === 0) return jsonResponse([]);
  ...
  const matchingRows = values.slice(1).filter(row => {
    ...
    return filterSpecs.every(spec => String(row[spec.index]).toLowerCase() === spec.value);
  });
...
function updateFormData(body) {
  ...
  const matchColIdx = headers.findIndex(h => h.toLowerCase() === matchColumn.toLowerCase());
  if (matchColIdx === -1) {
    return jsonResponse(null, `The lookup column '${matchColumn}' was not found in the sheet headers.`);
  }
  ...
  recordsToUpdate.forEach(item => {
    ...
    for (const columnKey in updateData) {
      const colIdx = headers.findIndex(h => h.toLowerCase() === columnKey.toLowerCase());
      
      // Prevent overwriting the match identifier column itself to safeguard data integrity
      if (colIdx !== -1 && colIdx !== matchColIdx) {
        sheet.getRange(targetRowIndex, colIdx + 1).setValue(updateData[columnKey]);
      }
    }
    updateCount++;
  });
...
function clearAllCache() {
  ...
  let keys_counter = 0
  keys.forEach((k) => {
    if (k.startsWith("FORM_") || k.startsWith("MAPPING_") || k.startsWith("HEADERS_")) {
      keys_counter++;
