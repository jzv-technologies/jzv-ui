...
const ParentSyllabusView = ({ student }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const [resBooks, resTrackers, resLogs, resSubjects, resClassifications] = await Promise.all(
          [
  ...
  const renderParentDailyActivity = () => {
    ...
    const mappedEntries = filteredItems.map((item) => {
      ...
      const lessonPath = [lesson?.level1, lesson?.level2, lesson?.level3]
        .filter(Boolean)
        .join(' ➔ ');
      return {
        id: item.id,
              ...
              {mappedEntries.length === 0 ? (
                ...
                mappedEntries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50/40">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">{getStatusBadge(entry.status)}</td>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
                {activeBooks.map((book) => {
                  ...
                  const bookLessons = syllabusData.filter(
                  ...
                  const activeBookLessons = bookLessons.filter((n) => {
                    ...
                    if (n.level2 && !n.level3) {
                      const hasL3 = bookLessons.some(
                        (o) =>
                          o.level1 === n.level1 && (o.level2 || 'General') === n.level2 && o.level3
                      );
                      return !hasL3;
                    }
                    if (n.level1 && !n.level2 && !n.level3) {
                      const hasL2orL3 = bookLessons.some(
                        (o) => o.level1 === n.level1 && (o.level2 || o.level3)
                      );
                      return !hasL2orL3;
                    }
                    return false;
                      ...
                      <div>
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex-1 min-w-0 text-left">
                            ...
                            <h4
                              className="text-xs font-black text-dark-primary truncate mt-0.5"
                              title={book.name}
                            >
                              {book.name}
                            </h4>
                          ...
                          <div
                            className={`flex items-center justify-center w-10 h-10 rounded-xl border ${pctBg} shrink-0 ml-2`}
                          >
              ...
              {expandedBookId && (
                <div className="bg-white border rounded-2xl shadow-sm p-5 text-left">
                  {(() => {
                    ...
                    const uniqueLevel1s = [
                      ...new Set(activeBookLessons.map((l) => l.level1).filter(Boolean)),
                    ];
                          ...
                          {uniqueLevel1s.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                              {uniqueLevel1s.map((lvl1) => {
                                const lvl1Lessons = activeBookLessons.filter(
                                  (l) => l.level1 === lvl1
                                );
                                ...
                                const progressPct =
                                  lvl1Total > 0 ? totalProgressSum / lvl1Total : 0;
                                const barColor =
                                  progressPct >= 70
                                    ? '#10b981'
                                    : progressPct >= 30
                                      ...
                                      <div className="text-left">
                                        ...
                                        {itemHistory.length > 0 && (
                                          <p className="text-[10px] text-gray-500 font-semibold mt-1">
                                            Last activity:{' '}
                                            {new Date(itemHistory[0].date).toLocaleDateString()}
                                            {itemHistory[0].teacher?.name &&
                                              ` by ${itemHistory[0].teacher.name}`}
                                          </p>
                                        )}
                                      </div>
