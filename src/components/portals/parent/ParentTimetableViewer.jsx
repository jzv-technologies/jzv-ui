...
const SUBJECT_COLORS = [
  { match: ["quran", "tahfeez", "arabic"], cls: "bg-emerald-50 text-emerald-800 border-emerald-200" },
  { match: ["math", "algebra"], cls: "bg-blue-50 text-blue-800 border-blue-200" },
  { match: ["science", "physics", "chemistry", "biology"], cls: "bg-teal-50 text-teal-800 border-teal-200" },
  ...
  { match: ["islamic", "deeniyat", "hadith", "fiqh"], cls: "bg-amber-50 text-amber-800 border-amber-200" },
  { match: ["social", "history", "geography"], cls: "bg-orange-50 text-orange-800 border-orange-200" },
...
const getBreakInfo = (period) => {
  ...
  } else if (name.includes('salah') || name.includes('prayer') || name.includes('dhuhr') || name.includes('asr')) {
  ...
  const fetchTimetable = async (cId) => {
    ...
    try {
      ...
      if (classRow && periodsRows && slotsRows !== null) {
        const subjectMap = {};
        (subjectsRows || []).forEach((s) => { subjectMap[String(s.id)] = s.name; });
        const teacherMap = {};
        (teachersRows || []).forEach((t) => { teacherMap[String(t.id)] = t.name; });
      ...
      let localData = null;
      if (raw) {
        try { localData = JSON.parse(raw); } catch (_) {}
      }

      const localClasses  = localData?.classes  || MOCK_CLASSES;
      const localPeriods  = localData?.periods  || MOCK_PERIODS;
      const localSlots    = (localData?.slots    && localData.slots.length > 0)    ? localData.slots    : MOCK_SLOTS;
      const localSubjects = (localData?.subjects && localData.subjects.length > 0) ? localData.subjects : MOCK_SUBJECTS;
      const localTeachers = (localData?.teachers && localData.teachers.length > 0) ? localData.teachers : MOCK_TEACHERS;
      ...
      if (matchedClass) {
        const classSlots = localSlots
          ...
          .map((s) => ({
            ...
            subject_name: localSubjects.find((sub) => String(sub.id) === String(s.subject_id))?.name || null,
            teacher_name: localTeachers.find((t)   => String(t.id)   === String(s.teacher_id))?.name || null,
          }));
        setTimetableData({
  ...
  if (!student) {
    return (
      <div className="text-center py-16 bg-white border border-light-border rounded-3xl p-8 shadow-sm">
        ...
        <p className="text-dark-muted text-sm mt-1">Please select a student profile to view the class schedule.</p>
      </div>
    );
  }
    ...
    return (
      <div className="text-center py-16 bg-white border border-light-border rounded-3xl p-8 shadow-sm">
        <div className="w-16 h-16 rounded-full bg-emerald-50 border-2 border-emerald-200 flex items-center justify-center mx-auto mb-5">
          <i className={`fas ${isPendingSetup ? "fa-clock" : "fa-calendar-plus"} text-2xl ${isPendingSetup ? "text-emerald-500" : "text-light-muted"}`} />
        </div>
  ...
  if (displayDays.length === 0) {
    return (
      <div className="text-center py-16 bg-white border border-light-border rounded-3xl p-8 shadow-sm">
        ...
        <p className="text-dark-muted text-sm mt-2 max-w-sm mx-auto">
          Today is not a school day. Please select a different day or view the full week.
        </p>
                ...
                return (
                  ...
                  >
                    ...
                    {displayDays.map((day) => {
                      if (isBreak) {
                        return (
                          <td key={day} className="py-1.5 sm:py-2 px-1 sm:px-2 border-b border-light-border text-center">
                            <div className="flex items-center justify-center gap-1 text-amber-600">
                              <i className={`fas ${breakInfo.icon} text-[10px] sm:text-xs`} />
                              <span className="text-[8px] sm:text-xs font-medium">{breakInfo.label}</span>
                            </div>
                          </td>
                        );
                      }
                      ...
                      if (!slot || !slot.subject_name) {
                        return (
                          <td key={day} className="py-1.5 sm:py-2 px-1 sm:px-2 border-b border-light-border">
                            <div className="flex items-center justify-center">
                              <span className="text-[8px] sm:text-[10px] text-dark-muted italic">Free</span>
                            </div>
                          </td>
                        );
                      }
                      ...
                      return (
                        <td key={day} className="py-1.5 sm:py-2 px-1 sm:px-2 border-b border-light-border">
                          ...
                          >
                            <div className="text-[9px] sm:text-[11px] font-bold leading-tight">{slot.subject_name}</div>
                            {slot.teacher_name && (
                              <div className="text-[7px] sm:text-[9px] opacity-80 mt-0.5 truncate">{slot.teacher_name}</div>
                            )}
                          </div>
                        </td>
                      );
