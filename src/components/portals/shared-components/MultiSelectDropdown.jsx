...
const MultiSelectDropdown = ({
  ...
  const displayedOptions = options.filter((opt) => {
    ...
    if (hasGender && activeGenderFilter && activeGenderFilter !== 'all') {
      const isFemale = opt.is_female === true || opt.is_male === false || opt.prefix === 'fa-female' || opt.gender === 'female';
      ...
      <div className="max-h-52 overflow-y-auto divide-y divide-gray-50">
        {displayedOptions.length === 0 ? (
          ...
          displayedOptions.map((opt) => {
            ...
            const isFemale = opt.is_female === true || opt.is_male === false || opt.prefix === 'fa-female' || opt.gender === 'female';
            const hasGenderInfo = opt.is_male !== undefined || opt.is_female !== undefined || opt.gender !== undefined || opt.prefix;
            const prefixIcon = opt.prefix || (hasGenderInfo ? (isFemale ? 'fa-female' : 'fa-male') : null);
            ...
            return (
              ...
              >
                ...
                />
                {prefixIcon && (
                  <i className={`fas ${prefixIcon} text-[9px] shrink-0`} style={{ color: prefixColor }} />
                )}
  ...
  return (
    <div className={`${fullWidth ? 'w-full' : 'relative inline-block'} ${className}`}>
      ...
      >
        ...
        <i className={`fas fa-chevron-${open ? 'up' : 'down'} text-[8px] ml-1 shrink-0 opacity-80`} />
      </button>
      {panel}
    </div>
  );
