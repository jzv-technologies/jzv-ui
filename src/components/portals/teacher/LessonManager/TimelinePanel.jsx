...
const TimelinePanel = ({
  ...
  const getWeekdayColorStyles = (date) => {
    ...
    switch (day) {
      case 1: return { bg: 'bg-indigo-50 border-indigo-100', text: 'text-indigo-800' };
      case 2: return { bg: 'bg-emerald-50 border-emerald-100', text: 'text-emerald-800' };
      case 3: return { bg: 'bg-rose-50 border-rose-100', text: 'text-rose-800' };
      case 4: return { bg: 'bg-amber-50 border-amber-100', text: 'text-amber-800' };
      case 5: return { bg: 'bg-purple-50 border-purple-100', text: 'text-purple-800' };
      default: return { bg: 'bg-gray-50 border-gray-200', text: 'text-gray-700' };
    }
  };
  const getWeekColorStyles = (index) => {
    const mod = index % 5;
    switch (mod) {
      case 0: return { bg: 'bg-indigo-50 border-indigo-100', text: 'text-indigo-800' };
      case 1: return { bg: 'bg-emerald-50 border-emerald-100', text: 'text-emerald-800' };
      case 2: return { bg: 'bg-rose-50 border-rose-100', text: 'text-rose-800' };
      case 3: return { bg: 'bg-amber-50 border-amber-100', text: 'text-amber-800' };
      case 4: return { bg: 'bg-purple-50 border-purple-100', text: 'text-purple-800' };
      default: return { bg: 'bg-gray-50 border-gray-200', text: 'text-gray-700' };
    }
  };
  ...
  const renderStatusBadge = (record, lesson = null, onReplanClick = null) => {
    ...
    if (statusWarning === 'violated') {
      return <span className="text-[10px] bg-red-100 text-red-700 border border-red-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm"><i className="fas fa-exclamation-triangle"></i> Violated</span>;
    }
    if (statusWarning === 'replan') {
      const clickableProps = onReplanClick ? { 
        onClick: (e) => { e.stopPropagation(); onReplanClick(record); }, 
        className: "cursor-pointer hover:bg-orange-200 text-[10px] bg-orange-100 text-orange-700 border border-orange-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm transition-colors",
        title: "Click to update date"
      } : { 
        className: "text-[10px] bg-orange-100 text-orange-700 border border-orange-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm" 
      };
      return <span {...clickableProps}><i className="fas fa-clock"></i> Re-plan Required</span>;
    }
    
    if (isRevision && record.status === 'completed') {
      return <span className="text-[10px] bg-purple-100 text-purple-700 border border-purple-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm">Revision</span>;
    }
    if (record.status === 'completed') return <span className="text-[10px] bg-emerald-100 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm">Completed</span>;
    if (record.status === 'in_progress') return <span className="text-[10px] bg-amber-100 text-amber-700 border border-amber-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm">In Progress ({record.completion_percentage}%)</span>;
    return <span className="text-[10px] bg-indigo-100 text-indigo-700 border border-indigo-200 px-1.5 py-0.5 rounded font-bold uppercase shrink-0 shadow-sm">Planned</span>;
  };
  ...
  const handleDropAssign = async (e, dropDateStr, isWeekMode = false) => {
    ...
    try {
      const existing = progressRecords.find(p => 
        ...
        String(p.lesson_id) === String(lessonId)
      );
  ...
  const handleUpdateReplan = async (record) => {
    ...
    try {
      ...
      if (updateMode === 'carry_forward') {
        ...
        newStart = replanDate;
        const startD = record.target_start_date ? new Date(String(record.target_start_date).split('T')[0]) : new Date();
        const endD = record.target_end_date ? new Date(String(record.target_end_date).split('T')[0]) : startD;
  ...
  const actionRequiredRecords = progressRecords.filter(p => {
    if (String(p.class_id) !== String(selectedClassId) || String(p.subject_id) !== String(selectedSubjectId)) return false;
    if (p.status === 'completed') return false;
    const today = getLocalISODate(new Date());
    const endD = p.target_end_date ? String(p.target_end_date).split('T')[0] : (p.target_start_date ? String(p.target_start_date).split('T')[0] : null);
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="p-3 border-b bg-gray-50 flex items-center justify-between shrink-0 shadow-sm z-10">
        ...
        <div className="flex items-center gap-3">
          {planningMode === 'date' && (
            <div className="flex items-center gap-2 bg-indigo-50/50 px-3 py-1 rounded-lg border border-indigo-100">
              <label className="text-[10px] font-black text-indigo-400 uppercase tracking-wider">Start From:</label>
              <input 
                type="date" 
                value={startDateStr}
          ...
          {planningMode === 'date' ? (
            <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-0.5 shadow-sm">
              <button onClick={() => setTimelineDays(p => Math.max(1, p - 1))} className="text-gray-400 hover:text-indigo-600 px-2 py-1"><i className="fas fa-minus text-[10px]"></i></button>
              <span className="text-[11px] font-bold text-gray-600 w-12 text-center">{timelineDays} Days</span>
              <button onClick={() => setTimelineDays(p => Math.min(30, p + 1))} className="text-gray-400 hover:text-indigo-600 px-2 py-1"><i className="fas fa-plus text-[10px]"></i></button>
            </div>
          ) : (
            <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-0.5 shadow-sm">
              <button onClick={() => setTimelineWeeks(p => Math.max(1, p - 1))} className="text-gray-400 hover:text-indigo-600 px-2 py-1"><i className="fas fa-minus text-[10px]"></i></button>
              <span className="text-[11px] font-bold text-gray-600 w-12 text-center">{timelineWeeks} Wks</span>
              <button onClick={() => setTimelineWeeks(p => Math.min(10, p + 1))} className="text-gray-400 hover:text-indigo-600 px-2 py-1"><i className="fas fa-plus text-[10px]"></i></button>
            </div>
          )}
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4 bg-gray-50/50 pb-24 relative">
        ...
        {actionRequiredRecords.length > 0 && (
          <div className="bg-orange-50/50 border border-orange-200 rounded-xl p-3 shadow-sm mb-6">
            <h3 className="text-sm font-bold text-orange-800 mb-3 flex items-center gap-2"><i className="fas fa-exclamation-circle"></i> Action Required</h3>
            <div className="space-y-2">
              {actionRequiredRecords.map(record => {
                ...
                return (
                  <div key={`action-${record.id}`} className="flex justify-between items-center p-2.5 bg-white border border-orange-200 rounded-lg shadow-sm">
                    <div className="flex-1 min-w-0 pr-2">
                      <div className="font-semibold text-gray-800 text-xs truncate" title={fullPath}>{fullPath}</div>
                      {updatingReplanId === record.id && (
                        <div className="mt-2 flex items-center gap-2">
                          <span className="text-[10px] font-bold text-gray-500">{updateMode === 'carry_forward' ? 'New End Date:' : 'New Start Date:'}</span>
                          <input 
                            type="date" 
                            value={replanDate}
                          ...
                          />
                          <button onClick={() => handleUpdateReplan(record)} className="text-[10px] bg-emerald-100 text-emerald-700 font-bold px-2 py-1 rounded hover:bg-emerald-200">Save</button>
                          <button onClick={() => setUpdatingReplanId(null)} className="text-[10px] bg-gray-100 text-gray-600 font-bold px-2 py-1 rounded hover:bg-gray-200">Cancel</button>
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {renderStatusBadge(record, lesson, (rec) => { setUpdatingReplanId(rec.id); setReplanDate(getLocalISODate(new Date())); setUpdateMode('replan'); })}
                      
                      {!updatingReplanId && (
                        <button onClick={() => onLogWork(record)} className="text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 p-1.5 rounded transition-colors text-[10px] font-bold uppercase tracking-wide border border-indigo-100 text-center">Log Work</button>
                      )}
                    </div>
                  </div>
                );
        ...
        {planningMode === 'date' && timelineDates.map(date => {
          const dateStr = getLocalISODate(date);
          const recordsForDate = progressRecords.filter(p => {
            if (!p.target_start_date || String(p.class_id) !== String(selectedClassId) || String(p.subject_id) !== String(selectedSubjectId)) return false;
          ...
          return (
            <div 
              ...
              onDragEnter={(e) => { e.preventDefault(); setDraggedOverTarget(dateStr); }}
              ...
              className={`bg-white rounded-xl p-3 shadow-sm transition-all cursor-pointer ${
                isSelected ? 'ring-2 ring-indigo-600 border border-transparent bg-indigo-50/20' : 'border border-gray-200'
            ...
            >
              {/* Card Header */}
              <div className={`flex justify-between items-center mb-3 border-b pb-2 flex-wrap gap-2 ${colorStyles.bg} -mx-3 -mt-3 p-2 rounded-t-xl`}>
                <span className={`font-bold flex items-center gap-1.5 text-sm ${colorStyles.text}`}>
                  {isSelected && <i className="fas fa-map-pin text-indigo-600 text-sm"></i>}
                  {date.toLocaleDateString(undefined, { weekday: 'long', month: 'short', day: 'numeric' })}
                  {isToday && <span className="text-[9px] font-bold bg-indigo-600 text-white px-1.5 py-0.5 rounded uppercase tracking-wider ml-1 shadow-sm">Today</span>}
                </span>
                <button
                  onClick={(e) => { 
                    e.stopPropagation(); 
                  ...
                  {recordsForDate.map(record => {
                    ...
                    return (
                      <div 
                        key={record.id} 
                        draggable={true}
                        onDragStart={(e) => { e.dataTransfer.setData('text/plain', String(record.lesson_id)); }}
                      ...
                      >
                        <div className="flex-1 min-w-0 pr-1">
                          <div className="font-semibold text-gray-800 text-xs leading-tight mb-1 truncate" title={fullPath}>{fullPath}</div>
                          {updatingReplanId === record.id ? (
                            <div className="mt-2 flex items-center gap-2">
                              <span className="text-[10px] font-bold text-gray-500">{updateMode === 'carry_forward' ? 'New End Date:' : 'New Start Date:'}</span>
                              <input 
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          <button onClick={(e) => { e.stopPropagation(); onLogWork(record); }} className="text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 p-1.5 rounded transition-colors text-[11px] font-bold" title="Log Work">
                            <i className="fas fa-list-check"></i>
                          </button>
                          {!updatingReplanId && record.status === 'planned' && (
                            <button onClick={(e) => { handleUnplanLesson(e, record); }} className="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-1.5 rounded transition-colors text-[11px]" title="Remove from plan">
        ...
        {planningMode === 'week' && timelineWeeksList.map((weekDate, index) => {
          ...
          return (
            <div 
              key={`week-${index}`}
              onClick={() => setActiveTargetWeek(prev => prev === weekStartStr ? null : weekStartStr)}
              onDragEnter={(e) => { e.preventDefault(); setDraggedOverTarget(weekStartStr); }}
              onDragOver={(e) => e.preventDefault()}
              onDragLeave={() => setDraggedOverTarget(null)}
              onDrop={(e) => handleDropAssign(e, weekStartStr, true)}
              className={`bg-white rounded-xl p-3 shadow-sm transition-all cursor-pointer ${
                isSelected ? 'ring-2 ring-indigo-500 border-transparent bg-indigo-50/20' : 'border border-gray-200'
              } ${isDraggedOver ? 'border-2 border-dashed border-indigo-500 bg-indigo-50' : ''}`}
            >
              <div className={`flex justify-between items-center mb-3 border-b pb-2 flex-wrap gap-2 ${colorStyles.bg} -mx-3 -mt-3 p-2 rounded-t-xl`}>
                <div className={`font-bold flex items-center gap-1.5 text-sm ${colorStyles.text}`}>
                  {isSelected && <i className="fas fa-map-pin text-indigo-600 text-sm"></i>}
                  Week Start: 
                  <input 
                    type="date" 
                    ...
                    onChange={e => setCustomWeekDates(prev => ({...prev, [index]: e.target.value}))}
                    className="ml-1 bg-white/50 border border-black/10 rounded px-1 py-0.5 text-xs outline-none cursor-pointer"
                  />
                  - End date: {formatCustomDate(endD)}
                </div>
                <button
                  onClick={(e) => { 
                    e.stopPropagation(); 
                    ...
                    return (
                      ...
                      >
                        <div className="flex justify-between items-center w-full">
                          <span className="font-semibold text-gray-800 text-xs break-words flex-1 pr-2 leading-tight" title={fullPath}>
                            {fullPath}
                          </span>
                          <div className="flex items-center gap-1 flex-shrink-0">
                            {renderStatusBadge(record, lesson)}
                            {renderDateUpdateIcon(record)}
                            <button onClick={() => onLogWork(record)} className="text-indigo-600 hover:text-indigo-800 bg-indigo-50 hover:bg-indigo-100 p-1.5 rounded transition-colors text-[11px] font-bold ml-1" title="Log Work">
                              <i className="fas fa-list-check"></i>
                            </button>
                        ...
                        {updatingReplanId === record.id && (
                          <div className="mt-2 pt-2 border-t border-gray-100 flex flex-wrap items-center gap-2" onClick={e => e.stopPropagation()}>
                            <span className="text-[10px] font-bold text-gray-500">{updateMode === 'carry_forward' ? 'New End Date:' : 'New Start Date:'}</span>
                            <input 
                              type="date" 
                              value={replanDate}
                            ...
                            />
                            <button onClick={() => handleUpdateReplan(record)} className="text-[10px] bg-emerald-100 text-emerald-700 font-bold px-2 py-0.5 rounded shadow-sm hover:bg-emerald-200">Save</button>
                            <button onClick={() => setUpdatingReplanId(null)} className="text-[10px] bg-gray-100 text-gray-600 font-bold px-2 py-0.5 rounded hover:bg-gray-200">Cancel</button>
                          </div>
                        )}
                      </div>
                    );
