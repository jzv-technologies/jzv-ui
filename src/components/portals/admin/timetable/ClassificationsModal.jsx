// src/components/portals/admin/timetable/ClassificationsModal.jsx
...
const ClassificationsModal = ({
  ...
  const activeClassification = classifications.find(
    (c) => String(c.id) === String(activeClsId)
  );
  ...
  const handleAddCls = async (e) => {
    ...
    const updated = [...classifications, { id: newId, name: newClsName.trim() }];
  ...
  const handleRename = async (id) => {
    ...
    if (classifications.some(c => String(c.id) !== String(id) && c.name.toLowerCase() === editingName.trim().toLowerCase())) {
      showToast("A classification with this name already exists.", "error");
      return;
    }

    const updated = classifications.map((c) =>
      String(c.id) === String(id) ? { ...c, name: editingName.trim() } : c
    );
    await onSaveClassifications(updated);
    setEditingId(null);
  };
  ...
  const handleDeleteCls = (cls) => {
    const mappedCount = subjects.filter((s) => String(s.classification_id) === String(cls.id)).length;
  ...
  const filteredSubjects = [...subjects]
    ...
    .filter((sub) => {
      ...
      if (hideMapped) {
        const isMappedToOther = sub.classification_id && String(sub.classification_id) !== String(activeClsId);
        ...
        <div className="bg-white rounded-[2rem] border border-light-border shadow-2xl max-w-4xl w-full h-[650px] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
          ...
          <div className="flex-1 flex min-h-0">
            ...
            <div className="w-1/2 border-r border-light-border flex flex-col p-5 bg-light-bg/10">
              ...
              {/* Add form */}
              <form onSubmit={handleAddCls} className="flex gap-2 mb-4 shrink-0">
                <input
                  type="text"
                  required
                  ...
                  className="flex-1 bg-white border border-light-border rounded-xl px-3 py-2 text-xs font-semibold text-dark-primary outline-none focus:ring-2 focus:ring-brand-soft"
                />
                <button
                  type="submit"
                  className="bg-brand-primary hover:bg-brand-dark text-white px-3 py-2 rounded-xl text-xs font-bold shadow-sm transition-all"
                >
                  Add
                </button>
              </form>
                            ...
                            <>
                              ...
                              <button
                                ...
                                className={`p-1 rounded opacity-0 group-hover:opacity-100 hover:bg-red-50 transition-all ${
                                  isActive ? "text-white hover:text-red-primary" : "text-red-primary"
                                }`}
                                title="Delete"
                              >
              ...
              {activeClassification ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto space-y-1.5 border border-light-border/40 rounded-2xl p-3 bg-light-lbg/10 pr-1.5">
                    {filteredSubjects.length === 0 ? (
                      <div className="text-center text-xs italic text-dark-muted py-12">
                        {searchQuery ? "No matching subjects found." : "No subjects configured in the school."}
                      </div>
                    ) : (
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-light-bg/5 rounded-2xl border border-dashed border-light-border">
                  ...
                  <h5 className="text-sm font-bold text-dark-deepblue">No Classification Selected</h5>
